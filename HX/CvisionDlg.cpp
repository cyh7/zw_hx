...
HTuple hv_min_length_width_line = edit_frame_length * 0.22;
HTuple hv_min_length_height_line = edit_frame_width * 0.22;
...
HObject  ho_image_left, ho_ImagePart_left, ho_ObjectSelected_left;
HObject  ho_GrayImage_left, ho_Regions_left, ho_BinImage1_left, ho_ImageMedian_left;
HObject  ho_Region_left, ho_RegionClosing_left, ho_ConnectedRegions_left, ho_RegionDilation1_left, ho_RegionDifference_left;
HObject  ho_RegionFillUp_left, ho_SelectedRegions_left, ho_RegionTrans_left;
HObject  ho_BinImage_left, ho_Edges_left, ho_ContoursSplit_left, ho_SelectedXLD1_left, ho_Region1_left;
HObject  ho_SelectedXLD_left, ho_RegionLines1_left, ho_RegionLines_left, ho_RegionBorder_left, ho_RegionDilation_left, ho_ContCircle_left;
HObject  ho_Image1_left, ho_image2_left, ho_ImageZoom_left, ho_ConnectedRegions1_left, ho_SelectedRegions1_left, ho_BinImage2_left;
HTuple hv_Row1_left, hv_Column1_left, hv_Row2_left, hv_Column2_left;
// Local control variables

HTuple  hv_WindowHandle_left, hv_Width_left, hv_Height_left, hv_Number_left, hv_Attrib_left, hv_Radius_left, hv_StartPhi_left, hv_EndPhi_left, hv_PointOrder_left;
HTuple  hv_Row11_left, hv_Column11_left, hv_Row22_left, hv_Column22_left, hv_Width1_left;
HTuple  hv_Height1_left, hv_RowBegin_left, hv_ColBegin_left, hv_RowEnd_left;
HTuple  hv_ColEnd_left, hv_Nr1_left, hv_Nc1_left, hv_Dist1_left, hv_Phi_left, hv_LineLength_left;
HTuple  hv_RowBegin1_left, hv_ColBegin1_left, hv_RowEnd1_left, hv_ColEnd1_left;
HTuple  hv_Nr_left, hv_Nc_left, hv_Dist_left, hv_Phi1_left, hv_Row_left_left, hv_Column_left_left, hv_Length_left, hv_Length1_left;
HTuple  hv_IsOverlapping_left, hv_Angle_left;
HTuple	Width_left, Height_left, hv_Area_left, hv_Row_left, hv_Column_left, hv_ROW_left, hv_COL_left, hv_Rad_left, hv_n_left, hv_i_left;
...
UINT locateleft(LPVOID pParam)
{
	...
	try
	{
		...
		GetImageSize(ho_image_l, &hv_Width_left, &hv_Height_left);
	}
	...
	ZoomImageSize(ho_image_l, &ho_ImageZoom_left, hv_Width_left / scale, hv_Height_left / scale, "constant");
	WriteImage(ho_ImageZoom_left, "bmp", 0, "left.bmp");
	//获取图像尺寸


	//更新裁剪框的坐标
	hv_Row1_left = scale * m_startPos_l_y;
	hv_Column1_left = scale * m_startPos_l_x;
	hv_Row2_left = scale * m_endPos_l_y;
	hv_Column2_left = scale * m_endPos_l_x;

	CropPart(ho_image_l, &ho_ImagePart_left, hv_Row1_left, hv_Column1_left, hv_Column2_left - hv_Column1_left,
		hv_Row2_left - hv_Row1_left);
	GetImageSize(ho_ImagePart_left, &hv_Width1_left, &hv_Height1_left);
	Rgb1ToGray(ho_ImagePart_left, &ho_GrayImage_left);
	Threshold(ho_ImagePart_left, &ho_Regions_left, (HTuple)hv_Threshold, 255);
	AreaCenter(ho_Regions_left, &hv_Area_left, &hv_Row_left, &hv_Column_left);

	if (hv_Area_left < 0.1 * (hv_Row2_left - hv_Row1_left) * (hv_Column2_left - hv_Column1_left))
	{
		check_l_over = 0;
		empty_location_data();
	...
	Connection(ho_Regions_left, &ho_ConnectedRegions_left);
	SelectShapeStd(ho_ConnectedRegions_left, &ho_SelectedRegions_left, "max_area", 70);

	if(zhuaneikuang_flag==true)
	{
		Boundary(ho_SelectedRegions_left, &ho_RegionBorder_left, "inner");
		//*膨胀
		DilationCircle(ho_RegionBorder_left, &ho_RegionDilation_left, 2.5);
		RegionToBin(ho_RegionDilation_left, &ho_BinImage_left, 255, 0, hv_Width_left, hv_Height_left);

		//在弧形区域进行边缘检测
		EdgesSubPix(ho_BinImage_left, &ho_Edges_left, "canny", 1, 20, 40);
		//分割边缘：线和圆
		//对检测的边缘进行分割，识别线或者圆'lines_circles'，
		SegmentContoursXld(ho_Edges_left, &ho_RegionDilation_left, "lines_circles", 5, 5, 8);
		//统计识别出圆或线的数量
		CountObj(ho_RegionDilation_left, &hv_Number_left);
		//储存拟合圆的圆心坐标和半径
		hv_ROW_left = HTuple();
		hv_COL_left = HTuple();
		hv_Rad_left = HTuple();
		hv_n_left = 0;
		{
			HTuple end_val40 = hv_Number_left;
			HTuple step_val40 = 1;
			for (hv_i_left = 1; hv_i_left.Continue(end_val40, step_val40); hv_i_left += step_val40)
			{

				//选择轮廓并根据特性确定是否拟合圆：* Attrib = -1 线段 0 椭圆 1圆
				SelectObj(ho_RegionDilation_left, &ho_ObjectSelected_left, hv_i_left);
				GetContourGlobalAttribXld(ho_ObjectSelected_left, "cont_approx", &hv_Attrib_left);
				if (0 != (hv_Attrib_left > 0))
				{
					//逼近结果生成一个圆轮廓
					FitCircleContourXld(ho_ObjectSelected_left, "ahuber", -1, 2, 0, 3, 2, &hv_Row_left, &hv_Column_left,
						&hv_Radius_left, &hv_StartPhi_left, &hv_EndPhi_left, &hv_PointOrder_left);
					//这里会生成大量的拟合圆，通过添加条件，选取自己需要的圆，这里
					//条件为半径，（可以自己注释条件，查看所有拟合圆结果 ）
					if (0 != (HTuple(hv_Radius_left < 100).TupleAnd(hv_Radius_left > 10)))
					{
						//生成轮廓
						GenCircleContourXld(&ho_ContCircle_left, hv_Row_left, hv_Column_left, hv_Radius_left, 0, HTuple(360).TupleRad(),
							"positive", 1.0);
						//记录圆的圆心坐标和半径信息
						hv_ROW_left[hv_n_left] = hv_Row_left;
						hv_COL_left[hv_n_left] = hv_Column_left;
						hv_Rad_left[hv_n_left] = hv_Radius_left;
						hv_n_left += 1;
						if (HDevWindowStack::IsOpen())
							DispObj(ho_ContCircle_left, HDevWindowStack::GetActive());
						GenRegionContourXld(ho_ContCircle_left, &ho_Region_left, "filled");
						DilationCircle(ho_Region_left, &ho_RegionDilation1_left, 5.5);
						Difference(ho_SelectedRegions_left, ho_RegionDilation1_left, &ho_RegionDifference_left);
						ho_SelectedRegions_left = ho_RegionDifference_left;
					}
				}
			}
		}
	...
	RegionToBin(ho_SelectedRegions_left, &ho_BinImage1_left, 255, 0, hv_Width_left, hv_Height_left);
	MedianImage(ho_BinImage1_left, &ho_ImageMedian_left, "square", hv_Filter_block_radius, "mirrored");
	Threshold(ho_ImageMedian_left, &ho_Region1_left, 128, 255);

	ShapeTrans(ho_Region1_left, &ho_RegionTrans_left, "convex");
	RegionToBin(ho_RegionTrans_left, &ho_BinImage_left, 255, 0, hv_Width1_left, hv_Height1_left);

	
	//检测亚像素边缘
	EdgesSubPix(ho_BinImage_left, &ho_Edges_left, "canny", 1, 20, 40);
	SegmentContoursXld(ho_Edges_left, &ho_ContoursSplit_left, "lines_circles", 0, 4, 2);

	SelectShapeXld(ho_ContoursSplit_left, &ho_SelectedXLD1_left, "height", "and", hv_min_length_width_line,
		5000);


	SelectShapeXld(ho_ContoursSplit_left, &ho_SelectedXLD_left, "width", "and", hv_min_length_height_line,
		5000);

	FitLineContourXld(ho_SelectedXLD_left, "tukey", -1, 0, 5, 2, &hv_RowBegin_left, &hv_ColBegin_left,
	...
	hv_LineLength_left = 1000;
	if (0 != (hv_ColBegin_left < hv_ColEnd_left))
	{
		hv_RowBegin_left += (hv_Phi_left.TupleSin()) * hv_LineLength_left;
		hv_ColBegin_left = hv_ColBegin_left - (((hv_Phi_left.TupleCos()).TupleFabs()) * hv_LineLength_left);
	}
	else
	{
		hv_RowEnd_left += (hv_Phi_left.TupleSin()) * hv_LineLength_left;
		hv_ColEnd_left = hv_ColEnd_left - (((hv_Phi_left.TupleCos()).TupleFabs()) * hv_LineLength_left);
	}


	GenRegionLine(&ho_RegionLines1_left, hv_RowBegin_left, hv_ColBegin_left, hv_RowEnd_left, hv_ColEnd_left);
	FitLineContourXld(ho_SelectedXLD1_left, "tukey", -1, 0, 5, 2, &hv_RowBegin1_left, &hv_ColBegin1_left,
	...
	if (0 != (hv_RowBegin1_left > hv_RowEnd1_left))
	{
		hv_RowEnd1_left = hv_RowEnd1_left - (((hv_Phi1_left.TupleFabs()).TupleSin()) * hv_LineLength_left);
		hv_ColEnd1_left += ((hv_Phi1_left / (hv_Phi1_left.TupleFabs())) * (hv_Phi1_left.TupleCos())) * hv_LineLength_left;
	}
	else
	{
		hv_RowBegin1_left = hv_RowBegin1_left - (((hv_Phi1_left.TupleFabs()).TupleSin()) * hv_LineLength_left);
		hv_ColBegin1_left += ((hv_Phi1_left / (hv_Phi1_left.TupleFabs())) * (hv_Phi1_left.TupleCos())) * hv_LineLength_left;
	}
	GenRegionLine(&ho_RegionLines_left, hv_RowBegin1_left, hv_ColBegin1_left, hv_RowEnd1_left, hv_ColEnd1_left);
	...
	hv_RowEnd1_left += hv_Row1_left;
	hv_ColBegin_left += hv_Column1_left;
	hv_ColBegin1_left += hv_Column1_left;
	hv_ColEnd_left += hv_Column1_left;
	hv_ColEnd1_left += hv_Column1_left;
	hv_Row_l += hv_Row1_left;
	hv_Column_l += hv_Column1_left;
	//将直角边端点写入全局变量中
	left_begin_p_y = hv_RowBegin_left[0].D() / scale;
	left_begin_p_x = hv_ColBegin_left[0].D() / scale;
	left_end_p_y = hv_RowEnd_left[0].D() / scale;
	left_end_p_x = hv_ColEnd_left[0].D() / scale;

	left_begin_p1_y = hv_RowBegin1_left[0].D() / scale;
	left_begin_p1_x = hv_ColBegin1_left[0].D() / scale;
	left_end_p1_y = hv_RowEnd1_left[0].D() / scale;
	left_end_p1_x = hv_ColEnd1_left[0].D() / scale;
...
HTuple  hv_Nr, hv_Nc, hv_Dist, hv_Phi1, hv_Length, hv_Length1;
...
void CvisionDlg::OnCollectAndLocate()
{
	...
	HANDLE threadleftlocate = AfxBeginThread(locateleft, this, THREAD_PRIORITY_NORMAL);
	HANDLE threadrightlocate = AfxBeginThread(locateright, this, THREAD_PRIORITY_NORMAL);
	//等待子线程结束
	do
	{
		Sleep(1);
